...
# from utils.general_utils import recover_symetric
...
class LERFField(nn.Module):
    def __init__(
        ...
        # print("tot_out_dims",tot_out_dims, clip_n_dims)
    ...
    @staticmethod
    def _get_encoding(start_res, end_res, levels, indim=3, hash_size=18):
    ...
    def get_outputs(self, gaussian_samples: GaussianModel, clip_scales, valid_gaussians_mask=None) -> Dict[LERFFieldHeadNames, Float[Tensor, "bs dim"]]:
        ...
        # print("input to MHE",x.shape)
        if x.shape[0] == 0:
            clip_pass = torch.zeros((0, x.shape[1]), device=x.device, dtype=x.dtype)
        else:
