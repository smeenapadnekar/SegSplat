...
from lerf.encoders.image_encoder import BaseImageEncoder #(BaseImageEncoder,BaseImageEncoderConfig)
...
class OpenCLIPNetwork(BaseImageEncoder):
    ...
    def get_relevancy_segmentation(self, embed: torch.Tensor) -> torch.Tensor:
        phrases_embeds = self.pos_embeds
        p = phrases_embeds.to(embed.dtype)  # phrases x 512
        output = torch.mm(embed, p.T)  # rays x phrases
        softmax = torch.softmax(output, dim=-1) # rays x phrases
        return  softmax # rays x phrases
