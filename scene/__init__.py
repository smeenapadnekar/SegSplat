...
from utils.graphics_utils import fov2focal, compute_scale_gaussian_by_project_pair_pcd
...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images, args.eval, args.object_path, args.n_views, random_init=args.random_init)
        ...
        for resolution_scale in resolution_scales:
            ...
            """
            Generating Pseudo Views
            #meena: Not required for now
            ...
            print("Number of pseudo cameras",len(pseudo_cams))

        if self.loaded_iter:
            print("loading gaussians from point_cloud.ply")
            self.gaussians.load_ply(os.path.join(self.model_path,
                                                           "point_cloud",
                                                           "iteration_" + str(self.loaded_iter),
                                                           "point_cloud.ply"))
        else:
            print("calling create from pcd")
            # scale_gaussian = None
