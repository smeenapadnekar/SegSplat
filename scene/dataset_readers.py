...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, path, rgb_mapping,objects_folder):
    cam_infos = []
    for idx, key in enumerate(sorted(cam_extrinsics.keys())):
        ...
        object_path = os.path.join(objects_folder,  'Annotations', image_name + '.png')
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path,
                image_name=image_name, width=width, height=height, mask=None, bounds=bounds, objects=objects)
        cam_infos.append(cam_info)
    # print(cam_infos)
    sys.stdout.write('\n')
    return cam_infos
...
def readColmapSceneInfo(path, images, eval, object_path, n_views=0, llffhold=8, random_init=False, train_split=False):
    # ply_path = os.path.join(path, "sparse/0/points3D.ply")
    # bin_path = os.path.join(path, "sparse/0/points3D.bin")
    """
    Original ply file from COLMAP
    """
    ply_path = os.path.join(path, str(n_views) + "_views/dense/fused.ply")

    """
    Original ply file from VGGSFM
    """
    # ply_path = os.path.join(path, str(n_views) + "_views/dense/visuals/sparse.ply")
    """
    ply file from mast3r
    """
    # ply_path = os.path.join(path, str(n_views) + "_views/dense/points3D.ply")
    try:
    ...
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir),  path=path, rgb_mapping=rgb_mapping, objects_folder=os.path.join(path,object_dir))
    ...
    if n_views > 0:
        ...
        print(len(train_cam_infos),n_views)
