...
def get_feat(self, images, feat_type):
    """
    ...
    feat_upsampler = torch.hub.load("mhamilton723/FeatUp", feat_type, use_norm=use_norm).to(self.device)     ## offline
    if 'featup' in original_feat_type:
        feats = feat_upsampler(images).permute(0, 2, 3, 1)
    else:
        feats = feat_upsampler.model(images).permute(0, 2, 3, 1)
    return feats
...
class ViTExtractor:
    ...
    def _extract_features(self, batch: torch.Tensor, layers: List[int] = 11, facet: str = 'key') -> List[torch.Tensor]:
        ...
        self._feats = self.model.get_intermediate_layers(batch,1)
        self._feats = self._feats[0][:,1:,:].reshape(B,H//self.p,W//self.p,-1)
        # self._feats = upsampler(self._feats,H,W)
